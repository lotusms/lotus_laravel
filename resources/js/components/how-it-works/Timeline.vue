<template>
	<div class="timeline mt-4">
		<div :class="`timeline-item ${item.side}`" v-for="(item, i) in items" :key="i">
			<div class="timeline-subtitle">
				<p class="white-text">{{ item.phase }}</p>
			</div>
			<div class="timeline-dot"></div>
			<div class="timeline-content">
				<div class="uk-card uk-card-secondary uk-dark">
					<div class="uk-card-header uk-flex-middle uk-grid-small" uk-grid>
						<h3 class="uk-card-title green-text playball">{{ item.title }}</h3>
					</div>
					<img :src="`/img/timeline/${item.icon}.svg`" :class="`icon ${item.icon}`" :alt="item.title" />
